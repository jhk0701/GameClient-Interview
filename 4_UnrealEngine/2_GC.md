...
## TWeakObjectPtr
* UPROPERTY() 매크로가 붙은 UObject*는 GC에 의해 참조됨을 추적할 수 있음
* 이는 해당 객체에 대해 소유권을 주장하고 있는 것이며, GC가 사용되는 환경에서 소유권을 주장하지 않고 약참조하기 위해서 TWeakObjectPtr를 사용
* 즉, GC와 함께 안전하게 약참조
    * 약참조이면서 동시에 댕글링 포인터 위험을 제거하는 것이 핵심 장점
    * 이는 순환참조도 방지 가능
    * IsValid() 메서드를 통해서 해당 객체가 살아있는지, 유효한지 체크하여 안전하게 쓸 수있습니 다.
```cpp
